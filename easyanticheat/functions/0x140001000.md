...
<br/>
On the other hand, if the opration `ebx` - `[rax]` is actually equal to zero, then the jump occurs into the basic block that follows at `0x140001042`. In here, some sort of (byte) array is compared to the `dil` register (part of `edi`).
```asm
.text:0000000140001042 cmp     cs:byte_14007B170, dil
.text:0000000140001049 jnz     loc_14000110B
```
Again, if the comparasion does not result in 0, a jump is taken to the basic block at `0x14000110B`. If not, then the control flow gets redirected to the block which follows at  `0x14000104F`.
...
@ tim: amd x86 aufmachen? Intel oder AMD synthax? wahrscheinlich amd...
@ tim: 1152920920491294759 ??
@ tim: "byte_14007B170", was ist das? bytearray? ---> cmp bytearray, dil
