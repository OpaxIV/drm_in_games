...
1402ae2af  jne     0x1402ae29e
```
The register `rdp` is added to itself, so the chance is rather slim of the loop being executed, since only 0 + 0 = 0 would be a valid calculation. Other operations would lead to a positive or negative result, but not zero).

Still, this assumption cannot be stated with absolute certainty, since a more detailed analysis would be needed.
