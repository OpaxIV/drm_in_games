...
### jb is not executed
This path is taken, if the preceding comparison returns a "false". It starts at `0x14036fff5` and ends at `0x140159ac6`.

### jb is executed
If the preceding comparison holds, hence resulting in a "true", the jump leading directly to `0x14016e830` is taken. There is not much to say about this path, since it's a direct jump to the exit point, which will be analyzed further in the next chapter.
