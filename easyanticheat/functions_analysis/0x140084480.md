...
Finally there is an usual beviour: Instead of returning or at least jumping to another address, the function takes a `140556894  jmp     rcx`. This is quite unusual, since normally a jump to an address or `jmp *%reg` (dereference register and jump to address) would be more usual. This could be a hint of this function being part of a broader picture, maybe being a handler which is jump from and to.
<br>

## Type of Obfuscated Code <a name="typeofobfuscation"></a>
There is definately some sort of obfuscation in this function. This can be further proved by looking at the pseudo C-code (or the disassembly in more detail).

### Arithmetic Encoding
There are multiple operations typical to arithmetic encodings. An example is the following:
<br>
```C
14016e846      void* rcx_2 = ((char*)rcx_1 + ((((char*)rsi + (rbp_3 << 1)) * rsi) + ((rbp_3 * rbp_3) - (rbx * rbx))));
```
<br>
Which contains mulitple XOR and bit shifting operations.
<br>
### Opaque Predicate
The sheer amount of jumps and unusual control flow paths could be a sign of the function being partially related to an opaque predicate. This point cannot be stated with absolute certainty, since a more detailed analysis would be needed. It would be neccessary to symbolically execute some parts of the disassembly and to check all the jumps and branches to see, if these are even being considered by the program. 
<br>
